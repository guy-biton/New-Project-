...
class User {
    static async create(username, email, passwordHash, salt) {
        // VULNERABLE: String Concatenation used here!
        const sql = `INSERT INTO users (username, email, password_hash, salt) VALUES ('${username}', '${email}', '${passwordHash}', '${salt}')`;
    ...
    static async findByUsername(username) {
        // VULNERABLE: String Concatenation used here!
        const [rows] = await db.query(`SELECT * FROM users WHERE username IN ('${username}')`);
        return rows[0];
    }
